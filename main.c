...
//#include "timer.h"
#include "soc_C6748.h"
clock_t start,stop;	//统计时间用
extern int Image_data1[];
extern int Image_data2[];
//int mydata[IMAGE_SIZE];	//原始图像为1024时用此存放下采样后的256数据
typedef struct ConerPoint	//角点
{
    int x, y, id;			//坐标，尺度，编号
}ConerPoint;
...
int main(void) {
	...
	//L1PCFG |= 0X04;
	//L1DCFG |= 0X04;
	//MAR192 = 1;										//地址0xc0000000-0xcfffffff允许Cache
	//Sample((int *)Image_data, mydata, 1024, 1024);	//原始图像1024*1024时用此函数下采样
